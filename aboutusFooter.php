<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>footer</title>
    <link rel="stylesheet" href="./css/output.css">
</head>
<body>
<footer class="bg-blue-600 text-white py-2">
    <div class="max-w-7xl mx-auto px-4 flex justify-between items-start relative">
        <!-- Company Links - Left aligned -->
        <div class="text-left">
            <h1 class="text-xl font-bold mb-3">Company</h1>
            <ul class="space-y-2">
                <li><a href="./index.php" class="hover:text-blue-200 transition-colors">Home</a></li>
                <li><a href="./contactus.php" class="hover:text-blue-200 transition-colors">Contact Us</a></li>
            </ul>
            <p class="text-xs text-blue-100 mt-4">© 2025 CoolCarters. All rights reserved</p>
        </div>

        <!-- Social Media - Centered with thicker border -->
        <div class="mt-4 absolute left-1/2 transform -translate-x-1/2 backdrop-blur-sm rounded-lg shadow-lg">
            <h2 class="text-sm font-medium mb-3 text-center">TO KEEP US CONNECTED</h2>
            <div class="flex justify-center space-x-4">
                <a href="#" class="hover:opacity-80 transition-opacity">
                    <img src="./images/instagram-svgrepo-com.svg" alt="Instagram" class="h-5 w-5">
                </a>
                <a href="#" class="hover:opacity-80 transition-opacity">
                    <img src="./images/facebook-svgrepo-com.svg" alt="Facebook" class="h-5 w-5">
                </a>
                <a href="#" class="hover:opacity-80 transition-opacity">
                    <img src="./images/x.svg" alt="Twitter" class="h-5 w-5">
                </a>
            </div>
        </div>
    </div>
</footer>
</body>
</html>